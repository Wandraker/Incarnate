...
marker = "    private volatile long pandaRollUntilTick = Long.MIN_VALUE;\n"
if marker not in text:
    raise SystemExit("session fields marker not found")
text = text.replace(
    marker,
    marker
...
action_marker = """    public void startPandaRoll(int ticks) { pandaRollUntilTick = controlTick + Math.max(1, ticks); }
...
"""
action_add = action_marker + """    public void startWardenSonic(UUID targetId, int chargeTicks) {
...
text = text.replace(action_marker, action_add, 1)
...
for oldi, newi in replace_imports.items():
    if oldi not in text:
        raise SystemExit(f"ability import marker not found: {oldi!r}")
    text = text.replace(oldi, newi, 1)
if "import java.util.UUID;\n" not in text:
    text = text.replace("import java.util.List;\n", "import java.util.List;\nimport java.util.UUID;\n", 1)

field_marker = "    private final int pandaRollLockTicks;\n"
if field_marker not in text:
    raise SystemExit("ability field marker not found")
text = text.replace(field_marker, field_marker + """    private final boolean wardenSonicEnabled;
...
text = text.replace(primary_marker, primary_marker + """        if (vessel instanceof Ghast ghast && ghastFireballEnabled) {
...
text = text.replace(secondary_marker, secondary_marker + """        if (vessel instanceof Warden warden && wardenSonicEnabled) {
...
text = text.replace(tick_marker, tick_marker + """        if (vessel instanceof Warden warden) {
...
text = text.replace(abort_marker, abort_marker + """        if (vessel instanceof Warden && session.wardenSonicTracked()) {
...
primary_label = '        if (vessel instanceof Creeper && creeperEnabled) return "fuse";\n'
if primary_label not in text:
    raise SystemExit("primary label marker not found")
text = text.replace(primary_label, primary_label + '        if (vessel instanceof Ghast && ghastFireballEnabled) return "ghast-fireball";\n', 1)
secondary_label = '        if (vessel instanceof Panda && pandaRollEnabled) return "panda-roll";\n'
if secondary_label not in text:
    raise SystemExit("secondary label marker not found")
text = text.replace(secondary_label, secondary_label + '        if (vessel instanceof Warden && wardenSonicEnabled) return "sonic-boom";\n', 1)
...
if insert_before not in text:
    raise SystemExit("ability method insertion marker not found")
text = text.replace(insert_before, methods + insert_before, 1)

ghast_generic = """        if (vessel instanceof Ghast) {
            return LargeFireball.class;
        }
"""
if ghast_generic not in text:
    raise SystemExit("generic ghast projectile mapping not found")
text = text.replace(ghast_generic, "", 1)
...
if "import org.bukkit.entity.Ghast;\n" not in text:
...
text = text.replace(bat_apply, bat_apply + """        if (mob instanceof Ghast ghast) {
            ghast.setCharging(false);
        }
""", 1)
...
text = text.replace(bat_restore, bat_restore + """        if (mob instanceof Ghast ghast && ghastCharging != null) {
...
text = text.replace(recover_bat, recover_bat + """        if (mob instanceof Ghast ghast) {
...
if clear_marker not in text:
    raise SystemExit("recovery clear marker not found")
...
"""
if panda not in text:
    raise SystemExit("panda config marker not found")
text = text.replace(panda, panda + """  warden-sonic:
    enabled: true
    range: 20.0
    damage: 10.0
